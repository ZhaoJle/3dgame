...
public class gameController : MonoBehaviour, SceneController, UserAction {
	...
	void Start()
	{
		actionManager = GetComponent<SceneActionManager> ();
	}

	void Awake() {
		Direct director = Direct.getInstance ();
	...
	private void loadCharacter() {
		for (int i = 0; i < 3; i++) {
			MyCharacterController cha = new MyCharacterController ("priest"+i);
		...
		for (int i = 0; i < 3; i++) {
			MyCharacterController cha = new MyCharacterController ("devil"+i);
			cha.setName("devil" + i);
			cha.setPosition (fromCoast.getEmptyPosition ());
			cha.getOnCoast (fromCoast);
			fromCoast.getOnCoast (cha);
